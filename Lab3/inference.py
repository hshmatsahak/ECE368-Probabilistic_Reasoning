...
def forward_backward(all_possible_hidden_states,
    ...
    for state in all_possible_hidden_states:
        if observations[0] is None:
            ...
            factor = observation_model(state)[observations[0]]
        if prior_distribution[state] * factor != 0:
            forward_messages[0][state] = prior_distribution[state] * factor
    ...
    for i in range(1, len(forward_messages)):
        ...
        for state in all_possible_hidden_states:
            if observations[i] is None:
                ...
                factor = observation_model(state)[observations[i]]
            curr = 0
            for key in forward_messages[i-1]:
                curr += forward_messages[i-1][key]*transition_model(key)[state]
            if curr*factor != 0:
                dist[state] = curr * factor
    ...
    for i in range(num_time_steps-2,-1,-1):
        ...
        for state in all_possible_hidden_states:
            ...
            for key in backward_messages[i+1]:
                ...
                curr += backward_messages[i+1][key] * factor * transition_model(state)[key]
            if curr != 0:
                dist[state] = curr
