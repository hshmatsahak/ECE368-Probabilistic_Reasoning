...
def learn_distributions(file_lists_by_category):
    ...
    for f in all_files:
        f_words = set(util.get_words_in_file(f))
        vocabulary = vocabulary.union(f_words)

    p_d = util.get_word_freq(spam_files)
    q_d = util.get_word_freq(ham_files)
...
def classify_new_email(filename,probabilities_by_category,prior_by_category):
    ...
    for word in counts:
        if word in p_d:
              spam_prob = spam_prob + counts[word]*np.log10(p_d[word])
              ham_prob = ham_prob + counts[word]*np.log10(q_d[word])

    spam_prob += np.log10(prior_by_category[0])
    ham_prob += np.log10(prior_by_category[1])

    # spam_prob_final, ham_prob_final = np.log10((10**spam_prob)/(10**spam_prob + 10**ham_prob)), np.log10((10**ham_prob)/(10**spam_prob + 10**ham_prob))
...
if __name__ == '__main__':
    ...
    for ratio in [1e-10, 1e-3, 1e-2, 1e-1, 0, 10, 20, 50, 100, 1e4, 1e5, 1e7, 1e10, 1e15, 1e20, 1e25, 1e50, 1e100, 1e400]:
        performance_measures = np.zeros([2,2])
        # Classify emails from testing set and measure the performance
        for filename in (util.get_files_in_folder(test_folder)):
            # Classify
            label,log_posterior = classify_new_email_mod(filename,
                                                    probabilities_by_category,
                                                    ratio)
    ...
    plt.xlim([-1,50])
    plt.ylim([-1,52])
    plt.title("Visualizing trade-off between type 1 and type 2 errors")
    plt.savefig("nbc.pdf")
    plt.show()
